...
function getLeaderboardData() {
  ...
  var ranked = demoIds.map(function(demoId) {
    ...
    // Top 3 aspects by frequency + avg aspects per voter (used as tiebreaker)
    var aspectCounts = {};
    var totalAspectSelections = 0;
    p.aspectCells.forEach(function(cell) {
      cell.split(",").forEach(function(a) {
        a = a.trim();
        if (a) {
          aspectCounts[a] = (aspectCounts[a] || 0) + 1;
          totalAspectSelections++;
        }
      });
    });
    ...
    // avgAspectsPerVoter: higher = voters found more things to praise = stronger signal
    var avgAspectsPerVoter = n > 0 ? totalAspectSelections / n : 0;
    ...
    return {
      ...
      avgAspectsPerVoter: Math.round(avgAspectsPerVoter * 100) / 100
    };
  });

  // Tiebreak order:
  //   1. Bayesian score (primary — protects well-voted projects)
  //   2. Raw avg score
  //   3. Avg aspects per voter (engagement depth — more aspects = more impressed voters)
  //   4. Unique feedbacks (more votes = more confident)
  ranked.sort(function(a, b) {
    return (b.bayesianScore      - a.bayesianScore)
        || (b.avgScore           - a.avgScore)
        || (b.avgAspectsPerVoter - a.avgAspectsPerVoter)
        || (b.uniqueFeedbacks    - a.uniqueFeedbacks);
  });
